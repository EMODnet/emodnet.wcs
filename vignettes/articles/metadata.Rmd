...
We can supply a [`<WCSClient>`](https://eblondel.github.io/ows4R/reference/WCSClient.html) object through the `wcs` argument to get compiled metadata from the server.
...
Let's take a closer look at the metadata in `coverage_details`. 
For details on these metadata, have a look at the `emdn_get_wcs_info()` help page (or use `?emdn_get_wcs_info` in R). 
...
You can get metadata for all EMODnet WCS services using `emdn_get_wcs_info_all()`.
...
You can get more detailed coverage metadata about specific coverage using `emdn_get_coverage_info()` and supplying a character vector of `coverage_ids`. 
Again, you can get the same information by supplying a service name to the `service` argument instead of a `<WCSClient>` object to the `wcs` argument.
...
The function returns a tibble of detailed coverage level metadata. 
...
The above functions compile and concatenate metadata into primarily tabular form for easier review. 
However the package offers a number of functions for extracting individual metadata in more usable forms.
...
Get the bounding box (geographic extent) of a coverage. 
Coordinates are given in the same Coordinate Reference System as the coverage.
The bounding box is approximate and might overestimate coverage.
...
This is useful to convert them to NA (see also `nil_values_as_na` argument of `emdn_get_coverage()`).
```{r}
...
Get the range of values of the data allowed in the bands of the coverage (for requests).
```{r}
